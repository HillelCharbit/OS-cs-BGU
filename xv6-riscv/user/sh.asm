...
0000000000000a62 <main>:
{
  ...
  while((fd = open("console", O_RDWR)) >= 0){
     ...
     a84:	3b6080e7          	jalr	950(ra) # e36 <open>
    ...
    if(fd >= 3){
     ...
     a96:	38c080e7          	jalr	908(ra) # e1e <close>
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    ...
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
     ...
     b1c:	34e080e7          	jalr	846(ra) # e66 <chdir>
...
gets(char *buf, int max)
{
  ...
  for(i=0; i+1 < max; ){
    ...
    cc = read(0, &c, 1);
     ...
     c62:	1b0080e7          	jalr	432(ra) # e0e <read>
    if(cc < 1)
...
stat(const char *n, struct stat *st)
{
  ...
  fd = open(n, O_RDONLY);
     ...
     cb4:	186080e7          	jalr	390(ra) # e36 <open>
  if(fd < 0)
  ...
  r = fstat(fd, st);
     ...
     cc4:	18e080e7          	jalr	398(ra) # e4e <fstat>
     cc8:	892a                	mv	s2,a0
  close(fd);
     cca:	8526                	mv	a0,s1
     ccc:	00000097          	auipc	ra,0x0
     cd0:	152080e7          	jalr	338(ra) # e1e <close>
  return r;
}
     cd4:	854a                	mv	a0,s2
...
malloc(uint nbytes)
{
  ...
  p = sbrk(nu * sizeof(Header));
    ...
    1304:	b7e080e7          	jalr	-1154(ra) # e7e <sbrk>
  if(p == (char*)-1)
